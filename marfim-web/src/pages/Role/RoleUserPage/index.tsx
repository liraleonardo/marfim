...
const RoleUserPage: React.FC = () => {
  ...
  const entity = useMemo(() => {
    return {
      name: 'Usuário do Perfil de Acesso',
      namePlural: 'Usuários do Perfil de Acesso',
      gender: 'M' as 'M' | 'F',
    };
  }, []);
  ...
  const handleSubmit = useCallback(() => {
    ...
    api
      ...
      .then(() => {
        addToast({
          type: 'success',
          title: `Usuários salvos com o perfil '${role.name}'`,
        });
        history.goBack();
      })
      .catch((err) => {
        handleError({ error: err, errorAction: 'associar usuário ao perfil' });
      })
      .finally(() => {
        setIsSubmiting(false);
      });
  }, [addToast, handleError, history, role.name, roleId, selectedUsers]);

  const tableHeader = (
    <div className="table-header">
      <h5 className="p-m-0">Gerenciar Usuários do Perfil</h5>
      ...
      {!isLoadingAllUsers && (
        ...
        >
          <DataTable
            ...
            ref={dt}
            value={allUsers}
            emptyMessage="Nenhum usuário encontrado para este perfil"
            dataKey="id"
